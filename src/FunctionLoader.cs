    ...
    internal class AzFunctionInfo
    {
        ...
        internal AzFunctionInfo(RpcFunctionMetadata metadata)
        {
            ...
            if (!string.IsNullOrEmpty(EntryPoint) &&
                !ScriptPath.EndsWith(".psm1", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private HashSet<string> GetParameters(string scriptFile, string entryPoint)
        {
            ...
            else
            {
                ...
                if (funcAsts.Count == 1)
                {
                    paramAsts = ((FunctionDefinitionAst) funcAsts[0]).Parameters;
                }
                else
                {
                    string errorMsg = funcAsts.Count == 0
